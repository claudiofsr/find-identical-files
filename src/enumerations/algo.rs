...
use crate::{Arguments, MyResult, open_file};
...
///
/// ### Returns
/// A `MyResult` containing the hash as a `String` if successful,
/// or an `Err(MyError)` if an I/O error occurs.
fn calculate_first_bytes_hash(file: &mut File) -> MyResult<String> {
...
fn get_foldhash<R>(mut reader: R) -> MyResult<String>
where
    R: Read,
{
    let mut buffer = [0_u8; BUFFER_SIZE];
...
mod tests_algo {
    ...
    fn create_temp_file(content: &[u8]) -> MyResult<NamedTempFile> {
    ...
    fn hash_bytes_with_algorithm(content: &[u8], algorithm: Algorithm) -> MyResult<String> {
    ...
    #[test]
    fn test_pathbuf_extension_first_bytes_hash() -> MyResult<()> {
    ...
    #[test]
    fn test_pathbuf_extension_full_file_hash() -> MyResult<()> {
        let content =
    ...
    #[test]
    fn test_pathbuf_extension_empty_file_full_hash() -> MyResult<()> {
